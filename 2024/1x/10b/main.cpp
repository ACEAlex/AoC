...
using Scoring = std::map<Coord, int>;
...
int64_t brute(Map &m, Coord c, int lvl, Scoring &scoreing) {
  // We scoored!
  if (lvl >= 9) {
    // m[c.second][c.first] = '.';
    scoreing[c]++;
  ...
  if (get_char(m, up) == '0' + lvl)
    ret += brute(m, up, lvl, scoreing);
  if (get_char(m, down) == '0' + lvl)
    ret += brute(m, down, lvl, scoreing);
  if (get_char(m, left) == '0' + lvl)
    ret += brute(m, left, lvl, scoreing);
  if (get_char(m, right) == '0' + lvl)
    ret += brute(m, right, lvl, scoreing);
