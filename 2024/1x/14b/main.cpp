...
int64_t solution_1(const Rules &rules, const Robots &robots) {
  ...
  for (int i = 0; i < 100000000; ++i) {
    Rules irule{rules.width, rules.height, i};
    std::vector<std::vector<char>> map(rules.height,
                                       std::vector<char>(rules.width, '.'));
    bool spread = true;
    for (const auto r : robots) {
      auto pos = r.move(irule);
      if (map[pos.y][pos.x] != '.')
        spread = false;
      map[pos.y][pos.x] = '*';
    ...
    // std::cout << "quad[0][0]: " << quad[0][0] << std::endl;
    // std::cout << "quad[0][1]: " << quad[0][1] << std::endl;
    // std::cout << "quad[1][0]: " << quad[1][0] << std::endl;
    // std::cout << "quad[1][1]: " << quad[1][1] << std::endl << std::endl;
    // if (quad[0][1] == 0 && quad[1][0] == 0  && quad[1][1] == 0)
    if (spread) {
