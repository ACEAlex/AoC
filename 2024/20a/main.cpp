...
int64_t is_cheat(const Map& m, const VisitedMap& visitMap, XY start, XY end, Cheats& cheats) {
  ...
  if (is_walkable(m, end)) {
    ...
    if (save >= (QueueItem::scorediff)) {
      std::cout << "Cheat: " << start << end << " save " << save << std::endl;
...
int64_t find_exit(const Map &m, Cheats& cheats) {
  ...
  while (!queue.empty()) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      next.cost += 1;
      next.pos += DirArray[i];
      if (is_walkable(m, next.pos) && (visitMap[next.pos.y][next.pos.x] == -1 ||
...
int main(int argc, char **argv) {
  ...
  Cheats cheats;
  auto steps_orig = find_exit(map, cheats);

  XY xy;
  for (xy.y = 0; xy.y < map.size(); ++xy.y)
  {
    for (xy.x = 0; xy.x < map[xy.x].size(); ++xy.x)
    {
      if (map[xy.y][xy.x] == '#')
      {
        map[xy.y][xy.x] = '.';
        auto steps_new = find_exit(map,cheats);
        if (steps_orig - steps_new >= QueueItem::scorediff)
        {
          sum++;
          cheats.insert({xy,xy});
        }
